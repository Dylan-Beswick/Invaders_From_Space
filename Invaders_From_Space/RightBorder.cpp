...
void RightBorder::Update(float DeltaTime)
{
	...
	for (vector<Collider*>::iterator Col = BorderCols.begin(); Col < BorderCols.end(); Col++) {
		if ((*Col)->GetOwner()->Tag == "Player") {
			Game::GetGameInstance()->PChara->SetMaxSpeed(); //- Didn't work/Crashed program
			...
			while (Game::GetGameInstance()->GameTimer != (DownTime + 0.5)) {
				Game::GetGameInstance()->EChara->SetMovementAxis(Vector2(0.0f, 1.0f));
			}

			Game::GetGameInstance()->EChara->SetMovementAxis(Vector2(-1.0f, 0.0f));
		}
	}
}
